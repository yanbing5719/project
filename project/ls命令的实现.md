...
```
结构体来实现对文件的一系列操作
 2. 整体思路就是**-r,-t**先来决定打印顺序，**-a**来确定要不要遍历隐藏文件，**-l,-s,-i**来决定打印格式。
 3. 实现打印格式的时候必须要把**-l,-s,-i**放到一个for循环里面实现，将**-l,-s,-i,-a,-r,-t**放到一个函数里面，最后在main函数里面分为有-R和没有-R两种情况来实现。
 4. 在判断文件类型时必须要借用stat结构体才可以获得文件类型信息
 5. 文件数组必须要动态分配内存，因为一般一个目录下面会有很多个文件。
 6. 我们在实现-R时要用到rewinddir让目录流的指针回到开头。
 7. 还有一个很重要的点就是我们在解析命令行参数的时候应该要注意路径名的拼接，否则他永远只对你指定的初始目录操作。
