...
//修改单个数据
b*change1(b*node,int c,int *num,float*num2,char str[]){
   printf("请输入要修改的内容\n");
    if(c==1||c==7||c==8){
        int obj;
        scanf("%d",&obj);
...
b*changebook(b*head){
    ...
    switch(c){
        case 1:{
        printf("id:%d\n",node->id);
        node=change1(node,1,&node->id,NULL,NULL);
        break;
        }
        case 2:{
        printf("书名：%s\n",node->name);
        node=change1(node,2,NULL,NULL,node->name);
        break;
        }
        case 3:{
        printf("作者：%s\n",node->author);
        node=change1(node,3,NULL,NULL,node->author);
        break;
        }
        case 4:{
        printf("出版社：%s\n",node->publisher);
        node=change1(node,4,NULL,NULL,node->publisher);
        break;
        }
        case 5:{
        printf("isbn: %s\n",node->isbn);
        node=change1(node,2,NULL,NULL,node->isbn);
        break;
        }
        case 6:{
        printf("价格：%.2f\n",node->price);    
        node=change1(node,6,NULL,&node->price,NULL);  
        break;
        }
        case 7:{
        printf("库存：%d\n",node->total);    
        node=change1(node,7,&node->total,NULL,NULL);
        break;
        }
        case 8:{
        printf("可借数量：%d\n",node->available);
        node=change1(node,8,&node->available,NULL,NULL);
        break;
        }
    }  
    return head; 
