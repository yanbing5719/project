...
## stdlib.h文件下的一些函数
- atoi 将字符串转化为数值，遇到第一个非数字字符就停止，不是数字返回0
- atof  将字符串转化为double 类型
- atol  将字符串转化为long 类型
...
## 作用域（程序中可访问标识符的区域）
- 块作用域
块是用一对花括号括起来的代码区域，定义在块中的变量具有块作用域。
```c
int main(){
}
```
...
- 外部链接
外部链接变量可以在多文件程序中可以用
外部链接文件作用域可以延展至其他翻译单元
- 内部链接
内链接文件作用域仅限于一个翻译单元
- 无链接（具有块作用域，函数原型作用域，函数作用域的变量）
**具有文件作用域的变量可以是外部链接或内部链接**               
## 存储期
- 静态存储期
程序执行期间一直存在，所有的文件作用域变量都有静态储存期，static声明其是内部链接
- 线程储存期
从声明时到线程结束一直存在。以关键字_Thread_local声明
- 自动储存期（局部变量）
块作用域的变量具有自动储存期。程序进入定义这些变量的块时，为这些变量分配内存，退出块时再释放。
**变长数组储存期从声明处到块的末尾，不是从块的开始到结束**
## 当内层块与外层块变量名相同时，内层块会隐藏外层块的定义，但是离开外层块后又回到了原来的作用域。
## 寄存器变量
储存在CPU寄存器中。与普通变量相比，访问和处理这些变量的速度更快，无法获取其地址。绝大多数方面与自动变量一样，使用**register**声明
## 全局变量与静态变量
| 特性        | 全局变量       | 静态变量                                   |
| --------- | ---------- | -------------------------------------- |
...
| **外部可见性** | 可以被其他文件访问  |  局部静态变量(函数内部声明）(无链接）：无法被外部访问<br> 全局静态变量（函数外部声明）（内部链接）：仅限于当前文件访问 |
## static
用于改变变量或函数的 存储周期 和 作用域。只分配一次内存，并在程序结束时才释放。（生命周期=整个程序运行过程）。
...
p可以被传递但是data一直存在。
- 内存分配到数据段。
如果一个定义在其他源代码文件的外部变量要在其他文件内使用，要用extren声明。extren只是引用，不可以定义外部变量。
## 存储类别说明符
auto register static extern  _Thread_local typedef
- extern
如果包含extern的声明具有文件作用域，引用变量必须要有外部链接，声明具有块作用域，可能具有外部链接或者内部链接。
## 生成一个随机数
```c
#include<stdio.h>
#include <stdlib.h>
...
## 访问文件的途径
- 文本模式
读取时把本地环境表示的行末尾或者文件结尾映射为C模式（\r转为\n)
写入时相反
- 二进制模式 
程序会看到文件中的(\n  \r)
## exit()
关闭所有打开的文件并结束程序
return 0     exit（0）
递归函数里面return 只是将控制权交给上一级，但是exit是直接结束程序
## 文件操作函数
- fopen(文件指针, 打开模式)
- fclose（文件名）成功返回0,否则返回EOF
- fgetc（文件指针）
- putc（字符，文件指针）
- fprintf（文件指针，......)与printf类似
- fscanf与上方类似
- fseek(FILE *stream, long offset, int origin)	移动文件位置指针	成功：0 失败：非0 
- ftell(FILE *stream)	获取当前文件位置	成功 当前位置  失败：-1L，返回long类型